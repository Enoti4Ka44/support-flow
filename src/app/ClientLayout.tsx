...
function NavItem({
  ...
  const isActive = pathname === path || (path === "/tickets" && pathname === "/");
  ...
  return (
    ...
    >
      <span className="text-lg flex shrink-0 items-center justify-center w-5 h-5">{icon}</span>
...
export function ClientLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-bg-main text-text-primary overflow-hidden font-sans w-full relative">
      ...
      >
        ...
        >
          ...
          <button
            ...
            className="hidden md:flex text-text-secondary hover:text-white p-1 rounded-md hover:bg-white/10 shrink-0 cursor-pointer"
      ...
      <main className="flex-1 flex flex-col px-4 md:px-6 py-4 md:py-6 mt-[65px] md:mt-0 overflow-y-auto w-full">
        {children}
      </main>
    </div>
  );
