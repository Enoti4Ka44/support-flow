      ...
      <div className="bg-bg-card rounded-2xl border border-border-dark overflow-hidden shadow-lg">
        ...
        <div className="p-6 border-b border-border-dark bg-white/[0.02]">
          <div className="flex flex-wrap items-center gap-3 mb-4">
            <span className="text-[13px] font-mono text-accent bg-accent/10 px-2 py-0.5 rounded">
              #{ticket.id}
            </span>
            <span
              className={`px-3 py-1 rounded-full text-[11px] font-medium tracking-[0.5px] ${status.bg} ${status.text}`}
            >
              {status.label}
            </span>
        ...
        <div className="p-6 border-b border-border-dark">
          ...
          <p className="text-text-primary/90 whitespace-pre-wrap leading-relaxed text-[15px]">
            {ticket.description}
          </p>
        </div>
        ...
        {ticket.ai_response && (
          <div className={`p-6 bg-accent/5 border-b border-border-dark`}>
            ...
            <div className="bg-black/20 rounded-xl p-4 border border-border-dark">
              <p className="text-text-primary/90 leading-relaxed text-[14px]">
                {ticket.ai_response}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            {(["open", "in_progress", "closed"] as const).map((s) => (
              <button
                ...
                className={`cursor-pointer px-5 py-2.5 rounded-xl text-[13px] font-medium transition-all ${
                  ticket.status === s
                    ? statusConfig[s].bg +
                      " " +
                      statusConfig[s].text +
                      " ring-1 ring-offset-2 ring-offset-bg-main ring-accent"
